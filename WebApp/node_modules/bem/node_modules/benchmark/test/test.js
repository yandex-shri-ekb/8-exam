...
(function (window, undefined) {
    ...
    var QUnit = window.QUnit || (
        window.setTimeout || (window.addEventListener = window.setTimeout = / /), window.QUnit = load('../vendor/qunit/qunit/qunit' + (platform.name == 'Narwhal' ? '-1.8.0' : '') + '.js') || window.QUnit, load('../vendor/qunit-clib/qunit-clib.js'), (window.addEventListener || 0).test && delete window.addEventListener, window.QUnit
        );

    /** The `Benchmark` constructor to test */
    var Benchmark = window.Benchmark || (
        Benchmark = load('../benchmark.js') || window.Benchmark, Benchmark.Benchmark || Benchmark
        );
    ...
    (function () {
        ...
        forOwn(objects, function (object, key) {
            test('clones ' + key + ' correctly', function () {
                var kind = toString.call(object), clone = Benchmark.deepClone(object);
        ...
        test('clones objects with circular references', function () {
            var object = createCircularObject(), clone = Benchmark.deepClone(object);
    ...
    (function () {
        ...
        test('allows a single source argument', function () {
            var source = { 'x': 1, 'y': 1 }, actual = Benchmark.extend({}, source);

            deepEqual(Benchmark.extend({}, source), { 'x': 1, 'y': 1 });
        });

        test('allows multiple source arguments', function () {
            var source1 = { 'x': 1, 'y': 1 }, source2 = { 'y': 2, 'z': 2 }, actual = Benchmark.extend({}, source1, source2);
    ...
    forOwn({
        ...
        (function () {
            ...
            test('emits with no listeners', function () {
                var event = Benchmark.Event('empty'), object = Constructor();
            ...
            test('emits with an event type of "toString"', function () {
                var event = Benchmark.Event('toString'), object = Constructor();
            ...
            test('returns the last listeners returned value', function () {
                var event = Benchmark.Event('result'), object = Constructor();
            ...
            test('aborts the emitters listener iteration when `event.aborted` is `true`', function () {
                var event = Benchmark.Event('aborted'), object = Constructor();
            ...
            test('cancels the event if a listener explicitly returns `false`', function () {
                var event = Benchmark.Event('cancel'), object = Constructor();
            ...
            test('uses a shallow clone of the listeners when emitting', function () {
                var event, listener2 = function (eventObject) {
            ...
            test('emits a custom event object', function () {
                var event = Benchmark.Event('custom'), object = Constructor();
    ...
    (function () {
        ...
        asyncTest('should abort deferred benchmark', function () {
            var fired = false, suite = Benchmark.Suite();
    ...
    (function () {
        ...
        test('supports sparse arrays as arguments', function () {
            var suite = Benchmark.Suite(), sparse = [];
    ...
    (function () {
        ...
        test('shifts an object with no elements', function () {
            var suite = Benchmark.Suite(), actual = suite.shift();
    ...
    (function () {
        var count = 0, suite = Benchmark.Suite();
    ...
    (function () {
        var events = [], callback = function (event) {
            events.push(event);
        };
    ...
    (function () {
        asyncTest('avoids call stack limits', function () {
            var result, count = 0, object = {}, recurse = function () {
                count++;
                recurse();
            };
