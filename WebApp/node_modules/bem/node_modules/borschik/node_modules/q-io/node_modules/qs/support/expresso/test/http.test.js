...
module.exports = {
    ...
    'test assert.response() delay': function (beforeExit) {
        ...
        assert.response(server, { url: '/delay', timeout: 1500 }, { body: 'delayed' }, function () {
            ++calls;
        });
    ...
    'test assert.response() regexp': function (beforeExit) {
        ...
        assert.response(server, { url: '/foo', method: 'POST', data: 'foobar' }, { body: /^\/foo foo(bar)?/ }, function () {
            ++calls;
        });
    ...
    'test assert.response() regexp headers': function (beforeExit) {
        ...
        assert.response(server, { url: '/' }, { body: '{"name":"tj"}', headers: { 'Content-Type': /^application\/json/ } }, function () {
            ++calls;
        });
    ...
    'test assert.response() with deferred listen()': function (beforeExit) {
        ...
        assert.response(delayedServer, { url: '/' }, { body: 'it worked' }, function () {
            ++calls;
        });
